...
chartpsr = parse.ChartParser(grammar) #parsed the grammar

def all_parses(wlist,lx):
    """returns all possible parse trees for all possible taggings of wlist"""
    allp = []
    for tagging in tag_words(lx, wlist): #1 tagging: [[Np, Np] [Np, Ns] [Np, Ts] [Ns, Ns]...] //6 combi out of 2 words.
        allp = allp + [t for t in chartpsr.parse(tagging)] #t in chartpsr.parse(tagging): ==a parse tree for a sentence 'tagging'
    return allp
...
#    N[s]   -> "Ns"  etc. print((label(all_parses(ww, lx))))
...
def N_phrase_num(tr):
    ...
    # the s or p from Ns or Np N,AN,Nom,AP
    ...
    if (tr.label() == 'Rel' and tr[0].label() == 'NP'):
        return N_phrase_num(tr[0])
